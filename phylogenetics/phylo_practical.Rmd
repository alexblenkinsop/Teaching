...
We can also visualise these data in R, re-coding ambiguous sites as 'others'. The x-axis is the position in the alignment, but this is not meaningful unless it is mapped to some known genomic coordinates. The y-axis are the taxa. We showw just the first 300 positions and first 20 taxa to keep it legible.

```{r, out.width='100%',fig.height = 8, fig.width=10, include=TRUE, echo=TRUE, eval=TRUE, fig.align="center"}

#ape::image.DNAbin(dna[,ape::seg.sites(dna)], cex.lab = 0.3, cex.axis = 0.7)
par(mar=c(5,8,4,2))
ape::image.DNAbin(dna[1:30,1:200], cex.lab = 0.5, cex.axis = 0.7)
```


## Generate the genetic distance matrix

Generate the distance matrix using the *ape* package, assuming the Tamura and Nei model for the evolutionary rate, which has two parameters. Plot the matrix.
...
What do you notice? Which tips share a common ancestor? Do any of the tree features suggest something has gone wrong with the alignment or phylogenetic inference?
...
Read in the new .fasta file and repeat the steps above. Now the distance matrix no longer shows large differences between all sequences and the misaligned sequence. The long branch has also gone from the tree.
...
Hint: the taxa labels for the alignment are stored in labels(dna).
...
We can root the tree using the command root(), and we can drop the root tip from the phylogeny as it isn't part of our study, using the drop.tip() function in *ape*.
...
lab.root <- labels(dna)[grep('Ref',labels(dna))]
...
Finally, we will not re-estimate the tree using a maximum-likelihood approach, which uses the sequence data in a statistical framework where we seek to estimate model parameters.

We will use the phangorn package, which requires the data to be in a different format, phyDat.

We then compute the likelihood of our existing neighbour-joining tree and optimise to find the best tree for the model parameters.


```{r, out.width='100%',fig.width=8, fig.height=8, include=TRUE, echo=TRUE, eval=TRUE, fig.align="center"}

# convert the alignment to PhyDat using phangorn package
#dat_phydat <- phyDat(dna, type = "DNA", levels = NULL)
dat2 <- as.phyDat(dna)
dat2
...
We next optimise, where the algorithm searches the tree topology space to find the best tree for the chosen substitution rate model.
...
We can then compare the optimal tree with the initial tree with a likelihood ratio test and confirm that it has a substantially larger likelihood.
...
#plotBS(midpoint(opt.tre.root), bs, type="p",cex = 0.6, tip.color = cols)
#bs <- bootstrap.pml(opt.tre.root, bs=100, optNni=TRUE, multicore=TRUE, control = pml.control(trace=0))
add.scale.bar()
title("ML tree")
